...
public class BajajDemoTestMobileWeb {
    ...
    public void takesMultipleSnapshotsInOneTestCaseForMobile() throws InterruptedException {
        ...
        // Set up mobile emulation capabilities for Mi Note 14 Pro
        Map<String, Object> deviceMetrics = new HashMap<>();
        deviceMetrics.put("width", 375);  // Match Figma width exactly (375px)
        deviceMetrics.put("height", 800); // Visible viewport height (actual device will scroll)
        deviceMetrics.put("pixelRatio", 2.75);
        deviceMetrics.put("mobile", true);

        Map<String, Object> mobileEmulation = new HashMap<>();
        mobileEmulation.put("deviceMetrics", deviceMetrics);
        mobileEmulation.put("userAgent", "Mozilla/5.0 (Linux; Android 13; Mi Note 14 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Mobile Safari/537.36");

        // Apply mobile settings to ChromeOptions if using Chrome
        if (driver instanceof ChromeDriver) {
            ChromeOptions chromeOptions = new ChromeOptions();
            chromeOptions.setExperimentalOption("mobileEmulation", mobileEmulation);
            // You may need to recreate the driver with these options
        }
        ...
        for (int i = 1; i <= lastRow; i++) {
            try {
                ...
                if (screenShotName != null && !screenShotName.isEmpty()) {
                    ...
                    options.put("timeout", 90000); // 90 seconds timeout for very long page

                    // Take Percy snapshot with width matching Figma exactly
                    percy.snapshot(screenShotName + "_mobile", java.util.List.of(375));

                    System.out.println("Mobile snapshot taken: " + screenShotName + "_mobile");
                }
            } catch (Exception e) {
